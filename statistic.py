...
def doccano():
    with open(args.dataset_path, 'r', encoding='utf-8') as f:
        ...
        sentense_len = []
        for i in lines:
            sentense_len.append(len(i['text']))
        print("句数:", len(sentense_len))
        print("最长单句样本长度:", max(sentense_len))
        freq = dict(Counter(sentense_len))
        count_large = 0
...
def w2ner():
    with open(args.dataset_path, 'r', encoding='utf-8') as f:
        ...
        sentense_len = []
        for i in data:
            sentense_len.append(len(i['sentence']))
            for j in i['ner']:
                for k in j['index']:
                    if k > 509:
                        print(i)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', choices=['bio', 'doccano', 'w2ner'], default="doccano",
                        help="select the data type [bio, doccano, w2ner].")
